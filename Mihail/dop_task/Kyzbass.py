...
def get_html(url, params=''):
    response = requests.get(url, headers=get_headers, params=params) #, headers=get_headers, params=params)
    return response

def get_content(html):
    global latlons, item, t
    city_list = [92734, 74054]  # спарсить список
    soup = BeautifulSoup(html, 'html.parser')
    # print(soup.prettify()) # делает красивое дерево для удобной работы
...
def get_headers(city_list):
    HEADERS = {}
    for city in city_list:  # пробрасывает кукис
        HEADERS.append(
            {
...
html = get_html(URL)
city_list = (get_content(html.text))
pprint(get_headers(city_list))


# def parser():
#     html = get_html(URL)
#     city_list = get_content(html.text)
#     for head in get_headers(city_list):
...
# parser()
        ...
        # for jj in HEADERS:
        #     # pprint(HEADERS.get(jj))
        #     html = get_html(URL, HEADERS.get(jj))
        # pprint(get_content(html.text))
    ...
    #     latlons_dict.update({'latlon': [latlon.get('data-lat'), latlon.get('data-lon')]})
    #     pprint(latlons_dict)
    #
    # for uuu in latlons_dict:
    #     pharmacy.append(
    #         {
    #             uuu: [latlons_dict.get(uuu)],
    #         }
    #     )
        # pprint(pharmacy)